...
def _profile(
    _, output_file=None, sort_by="cumulative", lines_to_print=None, strip_dirs=False
):
    ...
    def inner(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            with Path(_output_file).open("w") as f:
...
WHITESPACE_CHARS = " \t\r\n\f\v"
WHITESPACE = set(WHITESPACE_CHARS)
Num = int | float
INF = 10000
...
class Specification:
    ...
    width = default_width = (
        0.0  # w in the paper; the ideal width of the glue, the width of added typeset material for the penalty, or the static width of the box
    )
    stretch = default_stretch = (
        0.0  # y in the paper; the amount this glue can stretch/enlarge its width by
    )
    shrink = default_shrink = (
        0.0  # z in the paper; the amount this glue can shrink its width by
    )
    penalty = default_penalty = (
        0.0  # p in the paper; the amount to be penalized if use this penalty
    )
    flagged = default_flagged = (
        0  # f in the paper; used to say whether a hyphen will need to be put here. Is either 1 for True or 0 for False
    )
...
class KnuthPlassParagraph:
    ...
    def append_std_end(self):
        ...
        ends = [
            Penalty(
                0, INF, 0
            ),  # Forced non-break (must not break here, otherwise a Box coming before the Glue after this would allow a break to be here)
            Glue(
                0, 0, INF
            ),  # Glue that fills the rest of the last line (even if that fill is 0 width)
            Penalty(0, -INF, 1),
        ]  # Forced break (Ends last line)
    ...
    # @profile()
    def calc_knuth_plass_breaks(
        self,
        line_lengths: list[Num]
        | Num
        ...
        def compute_adjustment_ratio(pos1, pos2, line, line_lengths):
            ...
            if ideal_width < available_width:
                ...
                #   desired line
                y = (
                    sum_stretch[pos2] - sum_stretch[pos1]
                )  # The total amount of stretch (in whatever units all the parts of the paragraph are measured in) you can stretch this line by
                ...
                #   to fit on the line
                z = (
                    sum_shrink[pos2] - sum_shrink[pos1]
                )  # Total amount you could possibly shrink this line by to make it fit on the current desired line
        ...
        def active_nodes_gen():
            ...
            for line_num in sorted(active_nodes.keys()):
                yield from active_nodes[line_num]
        ...
        for i, b in enumerate(paragraph):
            ...
            if self.is_feasible_breakpoint(i):
                ...
                for a in active_nodes_gen():
                    r, desired_line_length = compute_adjustment_ratio(
                    ...
                    if -1 <= r <= tolerance:
                        ...
                        if r < -0.5:
                            fitness_class = 0  # tight line
                        elif r <= 0.5:
                            fitness_class = 1  # normal line
                        elif r <= 1:
                            fitness_class = 2  # loose line
                        else:
                            fitness_class = 3  # very loose line
        ...
        active_nodes = list(active_nodes_gen())
        ...
        breaks.pop(
            0
        )  # Ignore first break because it is break that started this paragraph, not first break in paragaph
    ...
    def line_contents(self, brk: Break):
        ...
        # end of the line broke at a space)
            ...
            brk.position + 1 if self.specs[brk.position].t == PENALTY else brk.position
        )
...
def insert_spaces(string, num_spaces):
    ...
    while True:
        ...
        for ch in string:
            if num_spaces > 0 and add_space and ch in WHITESPACE:
                if randint(  # noqa: S311
                    0, 1
                ):  # 50% chance to add space here, not for cryptographic use  # noqa: S311
                    out += " "
                    num_spaces -= 1
                added_space = True
                add_space = False
...
def str_for_breaks(par, breaks, justify: str = JUSTIFY.LEFT, end_mark: str = ""):
    ...
    for brk in breaks:
        ...
        if (justify == JUSTIFY.LEFT) or (
            justify == JUSTIFY.FULL and line_num == total_num_lines
        ):
            ...
            curr_line = (
                curr_line.strip()
            )  # May have whitespace on ends because of glues
...
medium_long_text = """Whether I shall turn out to be the hero of my own life, or whether that
